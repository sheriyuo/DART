...
class ModelService:
    ...
    def __init__(self, model_config: ModelConfig, tool_config: ToolConfig):
        ...
        self.twolora_paths = [
            "/text2sql/verl-tool/2lora_ckpts/search_r1_qa_em/search_r1_qa_em-fsdp-agent-_mnt_bn_data-agent_search_r1_base_model_qwen2.5-3b-sft100-2lora-grpo-n16-b256-64-t1.0-lr2e-5-fix_prefix_bug",
            "/text2sql/verl-tool/2lora_ckpts/search_r1_qa_em/search_r1_qa_em-fsdp-agent-_text2sql_verl-tool_sft_model_qwen2.5-7b-instruct-adddtags_v0-20251227-213930_checkpoint-100-2lora-grpo-n16-b256-64-t1.0-lr1e-5-debug/global_step_100/actor/huggingface"
        ]
    ...
    async def _postprocess_responses(self, outputs: torch.Tensor, action_step: int) -> torch.Tensor:
        ...
        for i in range(len(active_responses)):
            ...
            if active_finish_reasons[i] == "stop" and outputs.choices[i].stop_reason is not None:
                action_token = outputs.choices[i].stop_reason
                if type(action_token) == int:
                    action_token = self.tokenizer.decode(action_token)
                    print(f"[Warning] action_token is not str: {action_token}, convert to str.")
                active_responses[i] = active_responses[i] + action_token
                if self.tool_config.enable_mtrl:
    ...
    def load_model(self):
        ...
        if self.model_config.model in self.twolora_paths:
            vllm_args.extend(["--model-impl", "transformers"])
            vllm_args.extend(["--enforce-eager"])
