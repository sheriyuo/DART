...
class DataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch(
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            entropy = None
            lora1_mask = micro_batch.get("lora1_mask", None)
            ...
            if self.use_remove_padding:
                ...
                if lora1_mask is not None:
                    lora1_mask_rmpad = index_first_axis(
                        rearrange(lora1_mask.unsqueeze(-1), "b s ... -> (b s) ..."), indices
                    ).transpose(0, 1) # (1, total_nnz)
                else:
                    lora1_mask_rmpad = None
                ...
                if self.use_ulysses_sp:
                    ...
                    if is_vlm_model:
                        ...
                        if lora1_mask_rmpad is not None:
                            lora1_mask_rmpad, _, _ = ulysses_pad(
                                lora1_mask_rmpad,
                        ...
                        if lora1_mask_rmpad is not None:
                            lora1_mask_rmpad, _, _ = ulysses_pad_and_slice_inputs(
                                lora1_mask_rmpad,
                                sp_size=self.ulysses_sequence_parallel_size
                            )
                ...
                if lora1_mask_rmpad is not None:
                    extra_args["lora1_mask"] = lora1_mask_rmpad
                ...
                if self.use_fused_kernels:
                    extra_args["temperature"] = temperature
                    extra_args["return_dict"] = True
                if "lora1_mask" in micro_batch.keys():
                    extra_args["lora1_mask"] = micro_batch["lora1_mask"]
    ...
    def compute_log_prob(self, data: DataProto, calculate_entropy=False) -> torch.Tensor:
        ...
        def _get_micro_batches(data: DataProto) -> tuple[list, list | None]:
            ...
            if "lora1_mask" in data.batch.keys():
                select_keys.append("lora1_mask")
