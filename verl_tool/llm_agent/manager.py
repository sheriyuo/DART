...
class AgentActorManager:
    def __init__(
        ...
        if self.config.mask_type_tokens is not None:
            if os.path.exists(self.config.mask_type_tokens):
                with open(self.config.mask_type_tokens, 'r') as f:
                    mask_token_str = f.read()
                    if mask_token_str == "no_mask":
                        self.mask_type_tokens = []
                    else:
                        self.mask_type_tokens = [x for x in mask_token_str.split(',') if x]
                logger.info(f"Using action stop tokens: {self.mask_type_tokens}")
            else:
                raise ValueError(f"action_stop_tokens file not found: {self.config.mask_type_tokens}")
        else:
            self.mask_type_tokens = []

        print("##################")
        print(self.mask_type_tokens)

        self.lora_router_tokens = ["</search>"]
    ...
    async def _postprocess_responses(self, responses: Union[torch.Tensor, List[str]], action_step: int, rollout_messages: list) -> torch.Tensor:
        ...
        do_actions = []
        self.suffix2prefix = {"</search>":"<search>", "</answer>":"<answer>", "</information>":"<information>", "</sql>":"<sql>", "</observation>":"<observation>"}
        async with self.tokenizer_lock:
            ...
            need_lossmask_id, need_loramask_id = set(), set()
            lora1_mapping = defaultdict(list)
            for i in range(len(responses_str)):
                ...
                for k in range(len(self.mask_type_tokens)):
                    if self.mask_type_tokens[k] in responses_str[i] and self.suffix2prefix[self.mask_type_tokens[k]] in responses_str[i]:
                        # must wrappered in mask token
                        need_lossmask_id.add(i)
                
                for l in range(len(self.lora_router_tokens)):
                    if self.lora_router_tokens[l] in responses_str[i] and self.suffix2prefix[self.lora_router_tokens[l]] in responses_str[i]:
                        need_loramask_id.add(i)
            ...
            encoding = self._batch_tokenize(responses_str)
            responses = encoding['input_ids'].to(torch.int64)
            offset = encoding['offset_mapping']

            offset_mapping = defaultdict(list)

            for resp_id in list(need_lossmask_id):
                resp = responses_str[resp_id]
                for suffix in self.mask_type_tokens:
                    prefix = self.suffix2prefix[suffix]
                    start_token, end_token = self.find_token_bounds_in_full(
                        offset[resp_id], resp, prefix, suffix
                    )
                    offset_mapping[resp_id].append([start_token, end_token])

            for resp_id in list(need_loramask_id):
                for suffix in self.lora_router_tokens:
                    prefix = self.suffix2prefix[suffix]
                    start_tokens, end_tokens = self.get_lora1_mask(
                        responses[resp_id], prefix, suffix
                    )
                    for start_token, end_token in zip(start_tokens, end_tokens): 
                        lora1_mapping[resp_id].append([start_token+1, end_token+1]) # 和推理对齐，往后偏移1个token

        return responses, responses_str, do_actions, rollout_messages, list(need_lossmask_id), offset_mapping, lora1_mapping
    ...
    def find_token_bounds_in_full(
        ...
        prefix: str,
        suffix: str,
        protect_suffix: bool = False
    ) -> Tuple[int, int]:
        # 定位 prefix、suffix 的字符索引（优先左侧匹配）
        if prefix:
            p_idx = full.find(prefix)
            if p_idx == -1:
                # 找不到 prefix -> set prefix_end to 0 (等同于前面没有 prefix token)
                prefix_end = 0
            else:
                prefix_end = p_idx + len(prefix)
        else:
            prefix_end = -1

        if suffix:
            # 在 prefix_end 之后寻找 suffix
            s_idx = full.find(suffix, prefix_end)
            if s_idx == -1:
                # 找不到 suffix -> set to len(full) (等同于 suffix 在末尾)
                suffix_start = len(full)
                suffix_end = len(full)
            else:
                suffix_start = s_idx
                suffix_end = s_idx + len(suffix) # Calculate suffix end
        else:
            suffix_start = len(full)
            suffix_end = len(full)

        last_prefix_idx = -1
        first_suffix_idx = len(offsets)  # 默认越界

        for idx, (s, e) in enumerate(offsets):
            # 有些 tokenizer 用 (0,0) 表示 special tokens（BOS/EOS/...）
            if s == 0 and e == 0:
                continue

            # token 完全在 prefix 之前（不重叠）
            if e <= prefix_end:
                last_prefix_idx = idx
                continue

            # token 完全在 suffix 之后（不重叠）
            if protect_suffix:
                # 如果保护后缀，我们需要找到第一个起始位置在 suffix_end 之后的 token
                # 这样 first_suffix_idx 之前的切片就会包含整个 suffix
                if s >= suffix_end:
                    first_suffix_idx = idx
                    break
            else:
                # 原逻辑：找到第一个起始位置在 suffix_start 之后的 token
                # 这样切片会在 suffix 之前停止
                if s >= suffix_start:
                    first_suffix_idx = idx
                    break

        start_token = last_prefix_idx + 1
        
        if protect_suffix:
            # 如果需要包含后缀，end_token 就是找到的那个“后缀之后”的第一个 token 的索引
            # 这样切片 [start:end] 时会包含中间所有的内容
            end_token = first_suffix_idx
        else:
            # 原逻辑（不包含后缀）：向前退一格
            end_token = first_suffix_idx - 1
            
        return start_token, end_token

    def get_lora1_mask(self, input_ids: torch.Tensor, prefix: str, suffix: str):
        start_token, end_token = [], []

        prefix_token = self.tokenizer.convert_tokens_to_ids(prefix)
        suffix_token = self.tokenizer.convert_tokens_to_ids(suffix)
        
        start_token = torch.where(input_ids == prefix_token)[0].tolist()
        end_token = torch.where(input_ids == suffix_token)[0].tolist()
        return start_token, end_token
    ...
    def _loss_masked_concatenate_with_padding(self,
        ...
        need_mask_id: List = None,
        offset_mapping: Dict = None,
        lora1_mapping: Dict = None,
        prompt_lora_mask: torch.Tensor = None,
    ) -> torch.Tensor:
        ...
        # prompt means the old rsp part, response means the cur_rsp part
        ...
        response_with_mask = response.clone()
        for idx in offset_mapping:
            mask_range_list = offset_mapping[idx]
            for start, end in mask_range_list:
                response_with_mask[idx, start:end] = pad_id
        tensors_with_mask = [prompt_with_mask, response_with_mask]

        cur_lora1_mask = torch.zeros_like(response, dtype=torch.bool)
        for idx in lora1_mapping:
            mask_range_list = lora1_mapping[idx]
            for start, end in mask_range_list:
                cur_lora1_mask[idx, start:end] = 1  # mark the positions to be masked
        lora1_mask = [prompt_lora_mask, cur_lora1_mask]

        # info: observations, need to be masked
        if info is not None:
            # for non-masked tensors, just append the observation
            tensors.append(info)
            lora1_mask.append(torch.zeros_like(info,dtype=torch.bool))
        ...
        concatenated_lora1_mask = torch.cat(lora1_mask, dim=1)

        mask = concatenated != pad_id if pad_to_left else concatenated == pad_id
        sorted_indices = mask.to(torch.int64).argsort(dim=1, stable=True)
        padded_tensor = concatenated.gather(1, sorted_indices)
        padded_tensor_with_info = concatenated_with_info.gather(1, sorted_indices)
        padded_lora1_mask = concatenated_lora1_mask.gather(1, sorted_indices)

        return padded_tensor, padded_tensor_with_info, padded_lora1_mask
    ...
    def _update_right_side(
        ...
        if next_obs_ids != None:
            responses, responses_with_loss_mask, lora1_mask = self._loss_masked_concatenate_with_padding(
                right_side['responses'],
                right_side['responses_with_loss_mask'],
                cur_responses,
                next_obs_ids,
                pad_to_left=False,
                need_mask_id=need_mask_id,
                offset_mapping=offset_mapping,
                lora1_mapping=lora1_mapping,
                prompt_lora_mask=right_side['lora1_mask']
            )
        else:
        ...
        if self.config.truncate_response_side == 'left':
            ...
            return {'responses': responses[:, :max_len], 'lora1_mask': lora1_mask[:, :max_len],
    ...
    async def run_llm_loop_async(self, gen_batch: DataProto, **sampling_params: Dict[str, Any]) -> Tuple[Dict, Dict]:
        ...
        original_right_side = {'responses': initial_input_ids[:, []], "lora1_mask": torch.zeros_like(initial_input_ids[:, []], dtype=torch.bool),
        ...
        for step in range(self.config.max_turns+1):
            ...
            responses_ids, responses_str, do_actions, active_rollout_messages, need_lossmask_id, offset_mapping, lora1_mapping = await self._postprocess_responses(gen_output.batch['responses'], step, active_rollout_messages) # [active_size, ...]
    ...
    def _compose_final_output(
        ...
        if 'lora1_mask' in final_output: # [bs*n, max_response_length]
            curr_len = final_output['lora1_mask'].shape[1]
            target_len = self.config.max_response_length
            
            if curr_len < target_len:
                # 使用 F.pad 进行右侧填充，填充值为 0
                pad_amt = target_len - curr_len
                final_output['lora1_mask'] = F.pad(
                    final_output['lora1_mask'],
                    (0, pad_amt), # (left, right)
                    mode='constant',
                    value=0  # 强制填0
                )
            elif curr_len > target_len:
                # 理论上不应该发生，但防守式编程截断一下
                final_output['lora1_mask'] = final_output['lora1_mask'][:, :target_len]
        ...
        left_lora1_mask = torch.zeros_like(left_side['input_ids'], dtype=torch.bool) 

        for resp_id in range(left_side['input_ids'].shape[0]):
            for suffix in self.lora_router_tokens:
                prefix = self.suffix2prefix[suffix]
                start_tokens, end_tokens = self.get_lora1_mask(
                    left_side['input_ids'][resp_id], prefix, suffix
                )
                for start_token, end_token in zip(start_tokens, end_tokens):
                    left_lora1_mask[resp_id, start_token+1:end_token+1] = True # 和推理对齐，往后偏移1个token

        final_output['lora1_mask'] = torch.cat([
            left_lora1_mask, 
            final_output['lora1_mask']
        ], dim=1) # [bs*n, prompt_length + max_response_length]
