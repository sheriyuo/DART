...
class DatabaseManager:
    ...
    def execute_query(
        ...
        def _execute_query_inner():
            """Inner function to execute query with SQLite's progress handler timeout."""
            try:
                ...
                return f"Error executing SQL: {e}", None
        try:
            return func_timeout(timeout_ms / 1000, _execute_query_inner)
        except FunctionTimedOut:
            return f"SQL Timeout (process-level)", None
        except Exception as e:
            return f"Error executing SQL: {e}", None
