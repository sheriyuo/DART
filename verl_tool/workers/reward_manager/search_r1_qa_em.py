...
def compute_score_old(solution_str, ground_truth, method="strict", format_score=0.0, score=1.0, is_print=True):
    """
...
def compute_score(solution_str, ground_truth, method='strict', final_format_score=0, format_score=0., score=1., is_print=True):
    ...
    Args:
        ...
        score: the score for the correct answer
    """
    is_valid_format, out = is_valid_sequence(solution_str)
    answer = extract_solution(solution_str=solution_str)
    if is_print:
        do_print = random.randint(1, 64) == 1
    else:
    ...
    if do_print:
        ...
        if not is_valid_format:
            print(f"Invalid format: {out}")
            
    if answer is None:
        if is_valid_format:
            return format_score # 0.2
        else:
            return 0
    else:
