...
def _format_address_for_url(address: str) -> str:
    """
    Format address for URL, handling both IPv4 and IPv6 addresses.
    IPv6 addresses need to be enclosed in square brackets.
    """
    # Check if it's already a properly formatted IPv6 address with brackets
    if '[' in address and ']' in address:
        return address
    
    # Check if it's an IPv6 address (contains multiple colons)
    if address.count(':') > 1:  # IPv6 address will have multiple colons
        # Check if it already has a port (last part is digits)
        parts = address.split(':')
        if parts[-1].isdigit():  # Last part is a port number
            # Split into IP and port parts
            ip_part = ':'.join(parts[:-1])
            port_part = parts[-1]
            return f"[{ip_part}]:{port_part}"
        else:
            # No port, just wrap in brackets
            return f"[{address}]"
    return address  # IPv4 or already formatted address
...
class VerlToolChatCompletionScheduler(ChatCompletionScheduler):
    ...
    async def _chat_completions_openai(self, address: str, **chat_complete_request) -> ChatCompletion:
        formatted_address = _format_address_for_url(address)
        client = AsyncOpenAI(base_url=f"http://{formatted_address}/v1", api_key="token-abc123", timeout=None, max_retries=0)
    ...
    async def _chat_completions_aiohttp(self, address: str, **chat_complete_request) -> ChatCompletion:
        try:
            ...
            async with session.post(
                url=f"http://{formatted_address}/v1/chat/completions",
    ...
    async def _completions_aiohttp(self, address: str, **complete_request) -> Completion:
        try:
            ...
            session = aiohttp.ClientSession(timeout=timeout)
            async with session.post(
                url=f"http://{formatted_address}/v1/completions",
                headers={"Authorization": "Bearer token-abc123", **extra_headers},
    ...
    async def _abort(self, address: str, request_id: str) -> Dict[str, Any]:
        ...
        try:
            async with session.post(
                url=f"http://{formatted_address}/v1/abort",
